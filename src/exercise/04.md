...
Prop collections is basically what we already do in XUI, using
`{...spreadProps}` objects and occasionally composing functions like `onClick`s
via `composedOnClick` type functions.

Prop getters are an alternative approach that appears more valuable for
providing _functionality_ to others' components, rather than providing entire
components for consumers to use.

The idea is roughly the same, but instead of taking the props in to your
component and composing them internally, you export a 'getter' function that
takes in the consumer's custom props, and then composes them within that
function.

One super handy tool we could use is the custom `callAll` function that is
mentioned in the solutions. It looks like this:
...
This would allow us to more cleanly compose functions, like so:

```js
const onClick = callAll(props.onClick, toggle)
```
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Patterns%20%F0%9F%A4%AF&e=04%3A%20Prop%20Collections%20and%20Getters&em=alex%40surelee.dev).
